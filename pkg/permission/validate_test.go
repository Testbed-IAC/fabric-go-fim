...
func TestRequiredTags(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := RequiredTags(tc.req)
			for _, requirement := range got {
				if requirement.Tag == tc.tag {
					return
				}
			}
			t.Fatalf("RequiredTags = %+v, want tag %q", got, tc.tag)
		})
	}
}
...
func TestMissing(t *testing.T) {
	t.Parallel()
	req := Request{LifetimeHours: 48, Networks: []Network{{Type: "PortMirror"}}}
	missing := Missing(req, []string{TagSliceNoLimitLifetime})
	if len(missing) != 1 || missing[0].Tag != TagNetPortMirroring {
		t.Fatalf("Missing = %+v, want only %s", missing, TagNetPortMirroring)
	}
}
