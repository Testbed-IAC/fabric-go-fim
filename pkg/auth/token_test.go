...
func testJWT(t *testing.T, exp time.Time, projects []Project) string {
	t.Helper()
	header := base64.RawURLEncoding.EncodeToString([]byte(`{"alg":"none","typ":"JWT"}`))
	payload, err := json.Marshal(Claims{Exp: exp.Unix(), Projects: projects})
	if err != nil {
...
func TestParseJWTProjectClaims(t *testing.T) {
	t.Parallel()
	token := testJWT(t, time.Now().Add(time.Hour), []Project{{
		Name: "Project One",
		UUID: "project-1",
...
func TestStaticTokenExpiredErrors(t *testing.T) {
	t.Parallel()
...
func TestFileTokenRefreshesAndWritesFile(t *testing.T) {
	t.Parallel()
	var refreshCalls atomic.Int64
	newToken := testJWT(t, time.Now().Add(time.Hour), []Project{{
		Name: "Project One",
		UUID: "project-1",
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(TokenFile{IDToken: newToken, RefreshToken: "refresh-2"})
	}))
	defer server.Close()

	path := filepath.Join(t.TempDir(), "id_token.json")
	initial := TokenFile{
		IDToken:      testJWT(t, time.Now().Add(time.Minute), []Project{{Name: "Project One", UUID: "project-1"}}),
	...
	written, err := ParseTokenFile(path)
	if err != nil {
		t.Fatalf("ParseTokenFile returned error: %v", err)
	}
...
func TestFileTokenDoesNotRefreshFreshToken(t *testing.T) {
	t.Parallel()
	server := httptest.NewServer(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {
		t.Fatalf("unexpected refresh request")
	}))
	defer server.Close()
	token := testJWT(t, time.Now().Add(time.Hour), []Project{{UUID: "project-1"}})
	path := filepath.Join(t.TempDir(), "id_token.json")
	body, err := json.Marshal(TokenFile{IDToken: token, RefreshToken: "refresh-1"})
	if err != nil {
		t.Fatalf("marshal token file: %v", err)
	}
	if err := os.WriteFile(path, body, 0o600); err != nil {
